...
module.exports = async function index(req, res) {
  ...
  axios.all([
    ...
    axios.get('https://free.currconv.com/api/v7/convert?q=USD_UGX&compact=ultra&apiKey=3ea8e1b98e3f3fb48b2e')
  ]).then(axios.spread((waves, currency) => {
    const price = waves.data.USD * currency.data.USD_UGX;
    res.status(200).json({WAVES_UGX: price.toFixed(2)});
  }))
  .catch(error => {
    sails.log(error);
